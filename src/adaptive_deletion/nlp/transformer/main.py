...
flags.DEFINE_string('checkpoint_dir', '/tmp/haiku-lm1b', 'Directory to store checkpoints.')
...
def build_forward_fn(vocab_size: int, d_model: int, num_heads: int, num_layers: int, dropout_rate: float):
  ...
  def forward_fn(data: Mapping[str, jnp.ndarray], is_training: bool = True) -> jnp.ndarray:
    ...
    transformer = model.Transformer(num_heads=num_heads, num_layers=num_layers, dropout_rate=dropout_rate)
...
class CheckpointingUpdater:
  ...
  def init(self, rng, data):
    ...
    if not os.path.exists(self._checkpoint_dir) or not self._checkpoint_paths():
      ...
      checkpoint = os.path.join(self._checkpoint_dir, self._checkpoint_paths()[-1])
  ...
  def update(self, state, data):
    ...
    if step % self._checkpoint_every_n == 0:
      path = os.path.join(self._checkpoint_dir, 'checkpoint_{:07d}.pkl'.format(step))
...
def main(_):
  ...
  optimizer = optax.chain(optax.clip_by_global_norm(FLAGS.grad_clip_value), optax.adam(FLAGS.learning_rate, b1=0.9, b2=0.99))
