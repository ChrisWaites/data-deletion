...
def main(seed=0, dataset='projected_mnist'):
  pyrandom.seed(seed)
  ...
  alpha = 9.0
  perfect  = False
  learning_rates = [2 / (strong + smooth), 0.001, 0.01, 0.1, 0.5, 1, 5, 10]

  # Split train/test and create deletion sequence.
  X_train, y_train, X_test, y_test = projected_mnist()
  ...
  num_train, dim = X_train.shape[0], X_train.shape[1]
...
def parse_args():
  ...
  parser.add_argument('--dataset', type=str, default='projected_mnist', help='The name of the dataset.')
