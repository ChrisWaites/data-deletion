...
def clean_mnist(d =16, scale_and_center=True, intercept=False, normalize=True, sampling_rate=1):
    X = pd.read_csv('dataset/mnist_data/n=10000_d={}/X_test.csv'.format(d))
    X = X.sample(frac=sampling_rate)
...
def clean_adult_full(scale_and_center=True, intercept=True, normalize=True, sampling_rate=1.0):
    df = pd.read_csv('datasets/adult/adult_full.csv')      #full adult data
    ...
    df = df.drop('income', 1)
    # hot code categorical variables
    sens_df = pd.read_csv('datasets/adult/adult_protected.csv')
    sens_cols = [str(c) for c in sens_df.columns if sens_df[c][0] == 1]
    sens_dict = {c: 1 if c in sens_cols else 0 for c in df.columns}
